...
processFilterOptions(unsigned int const         filterSpec,
                     ...
                     struct cmdlineInfo * const cmdline_p) {

    if (filterSpec) {
        filter baseFilter;
        lookupFilterByName(filterOpt, &baseFilter);
        cmdline_p->filterFunction = baseFilter.function; 
        cmdline_p->filterRadius   = baseFilter.radius;

        if (windowSpec) {
            filter windowFilter;
            lookupFilterByName(windowOpt, &windowFilter);
            
            if (cmdline_p->windowFunction == filter_box)
                cmdline_p->windowFunction = NULL;
            else
                cmdline_p->windowFunction = windowFilter.function;
        } else {
            /* Default for most filters is no window.  Those that _require_
               a window function get Blackman.
               */
            if (baseFilter.windowed)
                cmdline_p->windowFunction = filter_blackman;
            else
                cmdline_p->windowFunction = NULL;
        }
    } else
        cmdline_p->filterFunction = NULL;
}
...
parseScaleParms(int                   const argc, 
    ...
    if (argc-1 < 1)
        ...
        cmdline_p->xscale = cmdline_p->yscale = atof(argv[1]);
        
        if (cmdline_p->xscale == 0.0)
...
typedef struct {
    ...
    */
    int    nWeight;  /* Number of elements in 'Weight' */
    WEIGHT *Weight;
...
static int
appendWeight (WLIST *WList, int index, double weight) {
    int n;
    ...
    WList->Weight = realloc(WList->Weight, sizeof(*(WList->Weight))*(n+1));
    if (WList->Weight == NULL)
        pm_error ("appendWeight: realloc(%d) failed", 
                  sizeof(*(WList->Weight))*(n+1));
