...
static void
readOffKey(const char           textline[],
    ...
    cp = malloc(lineLength + 1);  /* leave room for terminating NUL */
    if (cp == NULL) 
...
-----------------------------------------------------------------------------*/
    ...
    
    {
        ...
        cp = malloc(lineLength+1);  /* leave room for safety NUL */
        if (!cp) 
            pm_error("Unable to allocate memory for text chunks");
    ...
    while (!eol) {
        ...
        if (c == '\n' || c == EOF)
            ...
            if (cursor > allocated - 1 - 1) { /* leave space for safety NUL */
                allocated *= 2;
                textline = realloc(textline, allocated * sizeof(textline[0]));
                if (textline == NULL)
...
handleArrayAllocation(png_text **    const arrayP,
    ...
    if (commentIdx >= *allocatedCommentsP) {
        ...
        *arrayP = realloc(*arrayP, *allocatedCommentsP * sizeof((*arrayP)[0]));
        if (*arrayP == NULL) 
