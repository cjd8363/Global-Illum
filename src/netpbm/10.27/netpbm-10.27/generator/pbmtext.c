...
insert_characters(bit **        const bits, 
                  ...
                  struct font * const font_p, 
                  int           const topmargin, 
                  int           const leftmargin,
                  float         const intercharacter_space,
                  int           const lspace) {
/*----------------------------------------------------------------------------
   Put the text 'lp' into the image 'bits' using font *font_p and
...
static void
flow_text(struct text    const inputText,
          int            const width, 
          struct font *  const fontP, 
          float          const intercharacterSpace,
...
static void
truncate_text(struct text   const inputText, 
...
static void
get_text(const char          cmdline_text[], 
...
main(int argc, char *argv[]) {
    ...
    get_text(cmdline.text, fontP, &input_text);
    
    if (cmdline.nomargins) {
        vmargin = 0;
        hmargin = 0;
    } else {
        if (input_text.lineCount == 1) {
    ...
    if (cmdline.width > 0) {
        ...
        if (input_text.lineCount == 1) 
            flow_text(input_text, cmdline.width, fontP, cmdline.space, &lp);
        else
            truncate_text(input_text, cmdline.width, fontP, cmdline.space, 
                          &lp);
    ...
    /* Put the text in  */
    insert_characters(bits, lp, fontP, vmargin, hmargin + maxleftb, 
