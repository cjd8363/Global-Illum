 ...
 *
   ...
   http://www.ijs.si/software/snprintf/snprintf-2.2.tar.gz and made the
   following changes:

   - Eliminate arithmetic on void pointers to avoid compiler warnings

   - Add "N" to all the names to avoid collisions with standard C libraries

   - Change return type of asprintf to const char * and add strfree()
     to free that type.

   - Define the NEED_* macros so we get all the routines.   

   - Change the name of the source file from snprintf.c to nstring.c

   - #include the interface header file instead of duplicating its
     declarations.

   - removed use of va_copy, which is not available everywhere.

   - removed use of va_start in non-variable-argument functions to quiet
     compiler warning.
...
int vsnprintfN(char *str, size_t str_m, const char *fmt, va_list ap) {
    return portable_vsnprintf(str, str_m, fmt, ap);
}

int snprintfN(char * dest, size_t const str_m, const char * const fmt, ...) {

    va_list ap;
    va_start(ap, fmt);
    return portable_vsnprintf(dest, str_m, fmt, ap);
}
...
int asprintfN(const char **ptr, const char *fmt, /*args*/ ...) {
  ...
  str = (char *) malloc(str_m = (size_t)str_l + 1);
  if (str == NULL) { errno = ENOMEM; str_l = -1; }
  else {
    int str_l2;
    va_start(ap, fmt);
    str_l2 = portable_vsnprintf(str, str_m, fmt, ap);
    va_end(ap);
    assert(str_l2 == str_l);
  }
  *ptr = str;
...
int vasprintfN(const char **ptr, const char *fmt, va_list ap) {
  ...
  if (str == NULL) { errno = ENOMEM; str_l = -1; }
    ...
    int str_l2 = portable_vsnprintf(str, str_m, fmt, ap);
    assert(str_l2 == str_l);
  }
  *ptr = str;
  return str_l;
...
int asnprintfN (const char **ptr, size_t str_m, const char *fmt, /*args*/ ...) {
  va_list ap;
  int str_l;
  char *str;
  ...
  /* if str_m is 0, no buffer is allocated, just set str to NULL */
  if (str_m == 0) {  /* not interested in resulting string, just return size */
  } else {
    str = (char *) malloc(str_m);
    if (str == NULL) { errno = ENOMEM; str_l = -1; }
    else {
      int str_l2;
      va_start(ap, fmt);
