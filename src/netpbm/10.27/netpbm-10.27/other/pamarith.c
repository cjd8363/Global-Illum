...
enum category {CATEGORY_FRACTIONAL_ARITH, CATEGORY_BITSTRING, CATEGORY_SHIFT};
...
static sample
applyFunction(enum function const function,
              sample        const leftArg,
              sample        const rightArg,
              sample        const resultMaxval) {

    sample retval;
    double resultArith;
    unsigned int resultBit;

    switch (function) {
    case FN_ADD:
        resultArith = MIN(resultMaxval, leftArg + rightArg);
        break;
    case FN_SUBTRACT:
        resultArith = MAX(0, (int)leftArg - (int)rightArg);
        break;
    case FN_MULTIPLY:
        resultArith = leftArg * rightArg / resultMaxval;
        break;
    case FN_DIFFERENCE:
        resultArith = leftArg > rightArg ? 
            leftArg - rightArg : rightArg - leftArg;
        break;
    case FN_MINIMUM:
        resultArith = MIN(leftArg, rightArg);
        break;
    case FN_MAXIMUM:
        resultArith = MAX(leftArg, rightArg);
        break;
    case FN_MEAN:
        resultArith = (leftArg + rightArg) / 2.0;
        break;
    case FN_COMPARE:
        resultArith = 
            leftArg > rightArg ? 2 : leftArg < rightArg ? 0 : 1;
        break;
    case FN_AND:
        resultBit = leftArg & rightArg;
        break;
    case FN_OR:
        resultBit = leftArg | rightArg;
        break;
    case FN_NAND:
        resultBit = ~(leftArg & rightArg) & resultMaxval;
        break;
    case FN_NOR:
        resultBit = ~(leftArg | rightArg) & resultMaxval;
        break;
    ...
    if (functionCategory(function) == CATEGORY_FRACTIONAL_ARITH)
        retval = (sample) (resultArith + 0.5);
    else
        retval = resultBit;
        ...
        for (col = 0; col < outpam.width; ++col) {
            ...
            for (outplane = 0; outplane < outpam.depth; ++outplane) {
                ...
                tuplerowOut[col][outplane] = 
                    applyFunction(cmdline.function, 
                                  tuplerow1[col][plane1], 
                                  tuplerow2[col][plane2],
                                  outpam.maxval);

                assert(tuplerowOut[col][outplane] >= 0);
                assert(tuplerowOut[col][outplane] <= outpam.maxval);
            }
        }
