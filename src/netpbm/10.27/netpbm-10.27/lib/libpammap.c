...
static void
addColorToHash(tuple          const color, 
    ...
    */
    for (row = 0; row < pamP->height && !full; ++row) {
        ...
        for (col = 0; col < pamP->width && !full; ++col) {
            ...
            addColorToHash(*colorP, *tuplefreqhashP, &freqPam, maxsize, 
                           sizeP, &full);
        }
    }
...
-----------------------------------------------------------------------------*/
    ...
    tuple * rowbuffer;  /* malloc'ed */
        ...
        */
    tuple color;  
        /* The color being current being added, scaled to the new maxval */
    jmp_buf jmpbuf;
    jmp_buf * origJmpbufP;
...
/*----------------------------------------------------------------------------
   Compute the a tuple frequency table from a PAM image.  This is an
   ...
   how many tuple value we find (except, of course, we abort if we
