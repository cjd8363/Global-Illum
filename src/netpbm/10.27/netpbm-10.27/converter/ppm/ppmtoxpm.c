...
-----------------------------------------------------------------------------*/
    ...
    else if (strcmp(cmdlineP->input_filespec, "-") == 0)
        cmdlineP->name = "noname";
    else {
        static char name[80+1];
        char *cp;

        STRSCPY(name, cmdlineP->input_filespec);
        cp = strchr(name, '.');
        if (cp)
...
static unsigned int
xppMaxvalFromMaxval(pixval const maxval) {
...
/*----------------------------------------------------------------------------
   ...
   an XPP that has a colormap of size 'cmapSize'.  Each pixel in an XPP
   ...
   has fewer than 92 characters, each pixel will be represented by a
   single character.  If it has more than 92, but fewer than 92*92, it
   will take 2, etc.
-----------------------------------------------------------------------------*/
    unsigned int charsPerPixel;
    unsigned int j;

    for (charsPerPixel = 0, j = cmapSize; j; charsPerPixel++)
        j /= MAXPRINTABLE;
...
-----------------------------------------------------------------------------*/
    ...
    xppMaxval = xppMaxvalFromMaxval(maxval);
    ...
    for (cmapIndex = 0; cmapIndex < ncolors; ++cmapIndex) {
        ...
        if (colorname)
            ...
            PPM_DEPTH(scaledColor, color, maxval, xppMaxval);

            asprintfN(&hexString, mval == 0x000F ? "#%X%X%X" :
                      xppMaxval == 0x00FF ? "#%02X%02X%02X" :
                      xppMaxval == 0x0FFF ? "#%03X%03X%03X" :
                      "#%04X%04X%04X", 
