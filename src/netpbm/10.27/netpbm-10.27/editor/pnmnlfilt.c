...
static int 
atfilt_setup(double alpha, double radius, double maxscale) {
    ...
    if (alpha >= 0.0 && alpha < 1.0) {
        ...
        meanscale = maxscale * (-alpha/((1.0 - alpha) * 7.0));
            /* mean of 7 and scaled by -alpha/(1-alpha) */
        mmeanscale = maxscale * (1.0/(1.0 - alpha) + meanscale);    
...

    {
        ...
        */
        ...
        for (i=0; i <= MXIVAL; i++) {
            ...
            V0[i] = ROUNDSCALE(fi * v0);
            V1[i] = ROUNDSCALE(fi * v1);
            V2[i] = ROUNDSCALE(fi * v2);
            V3[i] = ROUNDSCALE(fi * v3);
            M0[i] = ROUNDSCALE(fi * m0);
            M1[i] = ROUNDSCALE(fi * m1);
            M2[i] = ROUNDSCALE(fi * m2);
            H0[i] = ROUNDSCALE(fi * h0);
            H1[i] = ROUNDSCALE(fi * h1);
            H2[i] = ROUNDSCALE(fi * h2);
            H3[i] = ROUNDSCALE(fi * h3);
        }
        /* set up alpha fraction lookup table used on big/small */
        for (i=0; i < (NOIVAL * 8); i++) {
            ALFRAC[i] = ROUNDSCALE((double)i * alphafraction);
        }
    }
    return alpharange;
}
