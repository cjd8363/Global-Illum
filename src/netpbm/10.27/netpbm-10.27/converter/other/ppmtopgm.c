...
static void
convertRaster(FILE * const ifP, int const cols, int const rows,
              pixval const maxval, int const format, 
              pixel * const inputRow, gray * const outputRow, 
              FILE * const ofP) {
    ...
    for (row = 0; row < rows; ++row) {
        ...
        if (maxval <= 255) {
            ...
            for (col = 0; col < cols; ++col) {
                pixel const p = inputRow[col];
                outputRow[col] = (gray)
                    ((times77[PPM_GETR(p)] + times150[PPM_GETG(p)] +
                      times29[PPM_GETB(p)] ) >> 8 );
            }
        } else {
