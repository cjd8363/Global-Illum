...
clusterConvertRow(struct converter * const converterP,
    ...
    unsigned int const radius = stateP->radius;

    unsigned int col;

    for (col = 0; col < converterP->cols; ++col)
        if (grayrow[col] > stateP->clusterMatrix[row % radius][col % radius])
            bitrow[col] = PBM_WHITE;
        else
            bitrow[col] = PBM_BLACK;
}
...
clusterDestroy(struct converter * const converterP) {
    ...
    for (row = 0; row < stateP->radius; ++row)
...
createClusterConverter(unsigned int const radius,
    ...
    MALLOCARRAY_NOFAIL(stateP->clusterMatrix, radius);
    for (row = 0; row < radius; ++row) {
        unsigned int col;

        MALLOCARRAY_NOFAIL(stateP->clusterMatrix[row], radius);
