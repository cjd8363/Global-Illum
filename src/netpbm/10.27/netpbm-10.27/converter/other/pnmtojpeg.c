        ...
        if (matched < 2)
            ...
            if (horiz <= 0 || horiz >= 1<<16)
                ...
                if (matched < 3) 
                    ...
                    if (strcmp(unitName, "dpi") == 0 || 
                        strcmp(unitName, "DPI") == 0)
                        densityP->unit = DEN_DOTS_PER_INCH;
                    else if (strcmp(unitName, "dpcm") == 0 || 
                        strcmp(unitName, "DPCM") == 0)
...
-----------------------------------------------------------------------------*/
    ...
    OPTENT3(0, "quality",     OPT_UINT,   &cmdline_p->quality, 
            &qualitySpec,        0);
    OPTENT3(0, "baseline",    OPT_FLAG,   NULL, &cmdline_p->force_baseline, 0);
    OPTENT3(0, "progressive", OPT_FLAG,   NULL, &cmdline_p->progressive,    0);
    OPTENT3(0, "arithmetic",  OPT_FLAG,   NULL, &cmdline_p->arith_code,     0);
    OPTENT3(0, "dct",         OPT_STRING, &dctval, NULL,                    0);
    OPTENT3(0, "grayscale",   OPT_FLAG,   NULL, &cmdline_p->grayscale,      0);
    OPTENT3(0, "greyscale",   OPT_FLAG,   NULL, &cmdline_p->grayscale,      0);
    OPTENT3(0, "rgb",         OPT_FLAG,   NULL, &cmdline_p->rgb,            0);
    OPTENT3(0, "maxmemory",   OPT_STRING, &maxmemory, NULL,                 0);
    OPTENT3(0, "tracelevel",  OPT_UINT,   &cmdline_p->trace_level, NULL,    0);
    OPTENT3(0, "qslots",      OPT_STRING, &cmdline_p->qslots,      NULL,    0);
    OPTENT3(0, "qtables",     OPT_STRING, &cmdline_p->qtablefile,  NULL,    0);
    OPTENT3(0, "sample",      OPT_STRING, &cmdline_p->sample,      NULL,    0);
    OPTENT3(0, "scans",       OPT_STRING, &cmdline_p->scans,       NULL,    0);
    OPTENT3(0, "smooth",      OPT_UINT,   &cmdline_p->smoothing_factor, 
            &smoothSpec,  0);
    OPTENT3(0, "optimize",    OPT_FLAG,   NULL, &cmdline_p->optimize,       0);
    OPTENT3(0, "optimise",    OPT_FLAG,   NULL, &cmdline_p->optimize,       0);
    OPTENT3(0, "restart",     OPT_STRING, &restart, NULL,                   0);
    OPTENT3(0, "comment",     OPT_STRING, &cmdline_p->comment, NULL,        0);
    OPTENT3(0, "exif",        OPT_STRING, &cmdline_p->exif_filespec, NULL,  0);
    OPTENT3(0, "density",     OPT_STRING, &density, 
            &cmdline_p->density_spec, 0);
    ...
    if (cmdline_p->rgb && cmdline_p->grayscale)
    ...
    if (cmdline_p->density_spec) 
        interpret_density(density, &cmdline_p->density);
