/* pbmtopsg3
   ...
   Reads a series of portable bitmaps and writes a postscript file
   containing these bitmaps as individual pages with Fax-G3 
...
static void
doPage(FILE * const ifp, int const page, double const dpi) {
    ...
    for (row = 0 ; row < rows ; row++ ) {
        ...
        for (j = 0 ; j < cols ; j++, cnt++ ) {
            if (bitrow[j] != lastbit ) {
                writelength (&outbits, &outbitsidx, 
                             &col, lastbit, cnt ) ;
                lastbit = 1 ^ lastbit ;
                cnt = 0 ;
            }
...
static void 
doPages(FILE * const ifp, int * const pagesP, double const dpi) {

    bool eof;
    unsigned int page;

    *pagesP = 0;  /* initial value */
