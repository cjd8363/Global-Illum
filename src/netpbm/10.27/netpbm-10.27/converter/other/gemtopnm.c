 ...
 * read 4-plane colour IMG files.  Therefore changed from PBM to PPM.
...
/* Support multiple planes by maintaining a separate row of bits for each
     ...
     int *planesP;
{
  ...
  if (pm_readbigshort (file, &s) == -1) /* Number of planes */
    pm_error ("EOF / read error");
  if (s != 4 && s != 1)
    pm_error ("This program only supports IMGs with 1 or 4 planes");
  *planesP = s;
