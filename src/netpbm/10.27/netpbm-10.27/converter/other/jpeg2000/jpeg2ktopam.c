...
createMatrices(struct pam * const outpamP, jas_matrix_t *** matrixP) {
    ...
    matrix = malloc(outpamP->depth * sizeof(jas_matrix_t *));
    if (matrix == NULL)
...
computeComponentMaxval(struct pam *  const outpamP,
    ...
    jasperMaxval = malloc(outpamP->depth * sizeof(int *));
    if (jasperMaxval == NULL)
        pm_error("Out of memory");
...
convertToPamPnm(struct pam *  const outpamP,
    ...
    jasperRow = malloc(outpamP->depth * sizeof(jas_seqent_t));
    if (jasperRow == NULL)
        pm_error("Out of memory");
