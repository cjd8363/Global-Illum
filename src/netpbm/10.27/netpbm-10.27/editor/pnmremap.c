...
-----------------------------------------------------------------------------*/
    ...
    unsigned int distanceDivider;
        /* The value by which our distance measurements have to be
           divided to make sure we don't overlow our 'int' data
           structure.
           
           To the extent 'distanceDivider' is greater than 1, we will
           return approximately the closest color -- there could
           conceivably be a closer one that we miss.  If
           'distanceDivider' is too small to ensure the calculation
           doesn't overflow, then we will pick an arbitrary color as
           closest, because of the possible arithmetic overflow.
        */
};
...
createColormapFinder(struct pam *             const pamP,
    ...
    {
        unsigned int const maxHandleableDiff = 
            (unsigned int)sqrt(UINT_MAX / pamP->depth);
        
        if (pamP->maxval > maxHandleableDiff)
            colormapFinderP->distanceDivider = 
                (unsigned int)(pamP->maxval / maxHandleableDiff + 0.1 + 1.0);
...
main(int argc, char * argv[] ) {
    ...
    /* Set up so input buffers have extra space as needed to convert the input
       to the output depth.
    */
