    ...
    if (backup) {
        ...
        if (strlen(line) == size - 1)
            pm_error("Input file has line that is too long (longer than "
                     "%d bytes).", size - 1);
        strncpy(lastInputLine, line, MAX_LINE);
    }
}
    ...
    */
    ...
    while ( !endOfEntry ) {
        ...
        if (strlen(str2) == 0)
            ...
            */
            for (key = 1; 
                 key <= NKEYS && strcmp(xpmColorKeys[key - 1], str2) != 0; 
                 key++);
            isKey = (key <= NKEYS);
...
static void
interpretXpmLine(const char line[], int const chars_per_pixel,
                 int const ncolors, int * const ptab, 
                 int ** const cursorP, int * const maxCursor) {
/*----------------------------------------------------------------------------
...
-----------------------------------------------------------------------------*/
    ...
    if (lineCursor == NULL)
        pm_error("No opening quotation mark in XPM input line which "
                 "is supposed to be a line of raster data: '%s'", line);

    ++lineCursor; /* Skip to first character after quote */

    /* Handle pixels until a close quote, eol, or the end of the image. */
    while (*lineCursor && *lineCursor != '"' && *cursorP <= maxCursor) {
        ...
        if (ptab == NULL)
            ...
            *(*cursorP)++ = colorNumber;
            ...
            /* colormap shadows ptab[].  Find this color number in ptab[] */
            int i;
