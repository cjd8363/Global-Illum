    ...
    long chunksize;
{
    ...
    if( verbose ) {
                ...
                case mskHasTransparentColor:
                    if( !maskfile )
                        pm_message("use '-maskfile <filename>' "
                                   ...
                                   mskNAME[bmhd->masking]);
                    break;
                case mskLasso:
                    pm_message("warning - masking type '%s' not supported", 
                               mskNAME[bmhd->masking]);
                    break;
                default:
        ...
        if( !maskfile )
            pm_message("use '-maskfile <filename>' "
                       "to generate a PBM mask file from genlock bits");
    }
    ...
    if( bmhd->xAspect != bmhd->yAspect ) {
        pm_message("warning - non-square pixels; "
            ...
            bmhd->xAspect > bmhd->yAspect ? 
                   (float)(bmhd->xAspect)/bmhd->yAspect : 
                   (float)(bmhd->yAspect)/bmhd->xAspect);
    }
    ...
    ColorMap *cmap;     /* for CLUTs */
{
    ...
    if( bmhd->compression != 4 )
        pm_error("invalid compression mode for %s: %d (must be 4)", 
                 ID2string(typeid), bmhd->compression);

    switch( typeid ) {
        case ID_RGBN:
            if( bmhd->nPlanes != 13 )
                pm_error("invalid number of planes for %s: %d (must be 13)", 
                         ID2string(typeid), bmhd->nPlanes);
    ...
    ColorMap *cmap;
{
    ...
    if( PCHG.Compression != PCHG_COMP_NONE ) {
        ...
        compsize = chunksize;
        MALLOCARRAY_NOFAIL(compdata, chunksize);
    ...
    if( PCHG.Flags & PCHGF_32BIT ) {
        ...
        pm_error("unknown palette changes structure format in %s chunk", 
                 ID2string(iffid));
...
process_body( FILE *          const ifp,
    ...
    if( typeid == ID_ILBM ) {
        ...
        *viewportmodesP |= fakeviewport;      /* -isham/-isehb */
        ...
        if( isdeepopt > 0 && (bmhd->nPlanes % 3 != 0) ) {
        ...
        if( isdeep > 0 )
            ...
            dcol_to_ppm(ifp, chunksize, bmhd, cmap, dcol);
        else if( bmhd->nPlanes > 8 ) {
            if( bmhd->nPlanes <= 16 && HAS_COLORMAP(cmap) )
                std_to_ppm(ifp, chunksize, bmhd, cmap, *viewportmodesP);
            else if( isdeep >= 0 && (bmhd->nPlanes % 3 == 0) )
                deep_to_ppm(ifp, chunksize, bmhd, cmap);
            else if( bmhd->nPlanes <= 16 )   
        ...
        ipbm_to_ppm(ifp, chunksize, bmhd, cmap, *viewportmodesP);
    else   /* RGBN or RGB8 */
        rgbn_to_ppm(ifp, chunksize, bmhd, cmap);
}
        ...
        *endchunkP = 1;
        ...
        case ID_BMHD:
            *bmhdP = read_bmhd(ifp, iffid, chunksize);
            break;
        case ID_CMAP:
            *cmapP = read_cmap(ifp, iffid, chunksize, *cmapP);
            break;
        case ID_CMYK:
            *cmapP = read_cmyk(ifp, iffid, chunksize, *cmapP);
            break;
        case ID_CLUT:
            *cmapP = read_clut(ifp, iffid, chunksize, *cmapP);
            break;
        case ID_CAMG:
            ...
            *viewportmodesP &= viewportmask;      /* -isnotham/-isnotehb */
            break;
        case ID_PCHG:
            *cmapP = read_pchg(ifp, iffid, chunksize, *cmapP);
            break;
        case ID_CTBL:
        case ID_SHAM:
            *cmapP = read_4bit_mp(ifp, iffid, chunksize, *cmapP);
            break;
        case ID_DCOL:
        ...
        case ID_BODY: 
            ...
            process_body(ifp, chunksize, *bmhdP, *cmapP, 
...
#define MAX_IGNORE  16
    ...
    if( cmaponly || (bmhd && bmhd->nPlanes == 0 )) {
        if( HAS_COLORMAP(cmap) ) {
            check_cmap(bmhd, cmap);
            cmap_to_ppm(cmap);
        } else
